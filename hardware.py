...
class MockHardwareController(BaseHardwareController):
    ...
    def set_rasp_in_pic(self, state: bool) -> None:
        ...
        self.logger.debug("RASP_IN_PIC (ACTJv20 RB6) -> %s", pin_state)
...
class GPIOHardwareController(BaseHardwareController):  # pragma: no cover - hardware dependent
    ...
    def __init__(self, pin_mode: str, pin_map: dict[str, int]) -> None:
        if GPIO is None:
        ...
        # ACTJv20(RJSR) legacy hardware compatibility: RASP_IN_PIC signal
        self.rasp_in_pic_pin = 12  # GPIO 12 -> ACTJv20 RB6 (RASP_IN_PIC)
    ...
    def set_rasp_in_pic(self, state: bool) -> None:
        ...
        try:
            ...
            GPIO.setmode(GPIO.BCM)
            # Setup pin as output with initial state
            GPIO.setup(self.rasp_in_pic_pin, GPIO.OUT, initial=GPIO.HIGH if state else GPIO.LOW)
            # Set the desired state
            GPIO.output(self.rasp_in_pic_pin, GPIO.HIGH if state else GPIO.LOW)
            self.logger.debug(f"RASP_IN_PIC (GPIO {self.rasp_in_pic_pin}) -> {'HIGH' if state else 'LOW'}")
        except Exception as e:
            self.logger.error(f"Failed to set RASP_IN_PIC GPIO {self.rasp_in_pic_pin}: {e}")
            raise
    ...
    def initialize_actj_gpio(self) -> None:
        ...
        try:
            ...
            # Initialize RASP_IN_PIC as ready (HIGH)
            ...
            self.logger.info(f"ACTJv20 GPIO initialized: RASP_IN_PIC (GPIO {self.rasp_in_pic_pin}) = HIGH (READY)")
...
def _create_controller() -> BaseHardwareController:
    ...
    if controller == "gpio":
        try:
            return GPIOHardwareController(HARDWARE_PIN_MODE, HARDWARE_PINS)
